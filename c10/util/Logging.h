...
#else // !C10_USE_GLOG
#error "only support google glog"
#endif // C10_USE_GLOG
...
[[noreturn]] void ThrowEnforceFiniteNotMet(
    const char* file,
    const int line,
    const char* condition,
    const std::string& msg,
...
void LogAPIUsage(const std::string& context);
...
void LogPyTorchDDPUsage(const DDPLoggingData& ddpData);

namespace detail {
// Return value is needed to do the static variable initialization trick
bool LogAPIUsageFakeReturn(const std::string& context);
} // namespace detail
